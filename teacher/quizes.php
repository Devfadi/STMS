...
if(isset($_POST["title"])){
	...
	//-------------------------------------------
	//ASSIGNMENT ID
	$result=$link->query("SELECT max(QID) AS ID from quiz");
		...
		while($row=$result->fetch_assoc()){
			...
			if($result1->num_rows>0){
				while($row1=$result1->fetch_assoc()){
					$uid=$row1["USERID"];
					mysqli_query($link,"insert into student_quizes(QID,USERID)values('$qid','$uid')");
				}
			}else{}
		}
	}else{}
	...
	//-------------------------------------------
	//-------------------------------------------
	mysqli_query($link, "insert into quiz(QID,TITLE,DESCRIPTION,FILE,COURSEID,ADDEDDATE,ADDEDBY,DUEDATE)values('$qid','$title','$description','$new_directory','$courseid','$currenttime','$activeuserid','$duedate')");
	$_SESSION['user_success_message'] = "The quiz has successfully been added";
...
if(isset($_POST["deletecourseid"])){
	...
	mysqli_query($link, "update quiz set VISIBILITY='DELETED' where QID='$courseid'");
	$_SESSION['user_success_message'] = "The quiz has successfully been deleted";
	$userurl='index.php?page=quizes';
	//echo '<script>window.location = "'.$userurl.'";</script>';
}else{}
...
<div class='row'>
	...
	<div class="col-sm-8">
		<table id="simple-table" class="table table-striped table-bordered table-hover no-margin-bottom no-border-top">
			...
			<tbody>
				<?php
				$result=$link->query("SELECT quiz.QID,quiz.TITLE,quiz.DUEDATE,quiz.FILE,quiz.ADDEDDATE,quiz.DESCRIPTION,course.NAME from quiz,course WHERE quiz.COURSEID=course.COURSEID AND quiz.VISIBILITY='ACTIVE' AND  quiz.ADDEDBY=$activeuserid order by QID DESC");
				if($result->num_rows>0){
					while($row=$result->fetch_assoc()){
						echo"
							<tr>
								...
								<td>
									<div class='hidden-sm hidden-xs btn-group'>
										<a href='#D".$row["QID"]."' role='button' class='red' data-toggle='modal'> 
